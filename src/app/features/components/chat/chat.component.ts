...
import { MessageInputComponent, MessageWithAttachment } from './message-input/message-input.component';
...
import { ChatSnapshotService } from '../../../service/chat-snapshot.service';
import { UuidFactoryService } from '../../../service/uuid-factory.service';
...
export class ChatComponent implements OnInit, OnDestroy {
    ...
    setCornerMenuHandlers() {
        this.cornerMenuItems = [{ label: 'Logout', icon: 'pi pi-sign-out', command: () => this.logout() }];
    }
    ...
    setUsersListHandlers() {
        ...
        combineLatest([this.snapshotService.getClientsList$(), this.typingService.getTypingMap$()])
            .pipe(sampleTime(700))
    ...
    private setMessageHistoryHandler() {
        // fires when a scroll position reaches the top of the chat
        this.scrollEmitter
            .pipe(
                debounceTime(200),
                filter((pos) => pos === 0),
