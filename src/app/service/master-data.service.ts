...
interface IApiClient {
    ...
    /**
     * Get the specific product data corresponding to the BookId
     */
...
interface IBookClient {
    ...
    /**
     * Get the random five books from the category of product whose BookId is supplied
     */
...
class BookClient implements IBookClient {
     ...
     */
    getCategoriesList(): Observable<void> {
        let url_ = this.baseUrl + "/api/Product/GetCategoriesList";
        url_ = url_.replace(/[?&]$/, "");
     ...
     */
    getSimilarBooks(bookId: number): Observable<void> {
        let url_ = this.baseUrl + "/api/Product/GetSimilarBooks/{bookId}";
        if (bookId === undefined || bookId === null)
            throw new Error("The parameter 'bookId' must be defined.");
        url_ = url_.replace("{bookId}", encodeURIComponent("" + bookId));
        url_ = url_.replace(/[?&]$/, "");
...
class Product implements IBook {
    bookId?: number;
