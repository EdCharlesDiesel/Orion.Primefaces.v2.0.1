...
@Component({
  ...
  template: `
    <ng-container>
      <div *ngIf="root && item.visible" class="layout-menuitem-root-text">{{ item.label }}</div>
      <a *ngIf="(!item.routerLink || item.items) && item.visible" [attr.href]="item.url"
      ...
      <a *ngIf="(item.routerLink && !item.items) && item.visible" (click)="itemClick($event)"
         [ngClass]="item.class"
      ...
      <ul *ngIf="item.items && item.visible" [@children]="submenuAnimation">
